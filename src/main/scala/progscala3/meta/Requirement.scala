...
object RequirementFailure:
  ...
  def apply(predicateString: String, message: String): RequirementFailure =
    new RequirementFailure(s"Requirement failure! predicate false: $predicateString. $message")
...
object requirement:
   ...
   */
  inline def apply(inline predicate: => Boolean): Unit =
    apply(predicate, "")
   ...
   */
  inline def apply(inline predicate: => Boolean,
                   inline message: String): Unit =
    ${ checkReq('predicate, 'message) }
