...
object invariant:
  ...
  inline def apply[T](inline predicate: => Boolean, message: => String = "")   // <2>
                     (inline block: => T): T =
  ...
  inline private def fail[T](inline predicate: => Boolean,
  ...
  private def failImpl[T](predicate: Expr[Boolean], message: Expr[String],
